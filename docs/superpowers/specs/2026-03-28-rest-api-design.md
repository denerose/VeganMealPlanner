...
- **Auth:** **`POST /api/auth/register`** and **`POST /api/auth/login`** are **unauthenticated**. **`POST /api/auth/logout`** requires **valid identity** (Bearer or dev header) but **not** **`HouseholdMembership`**. Full shape and rules are in [`2026-03-28-auth-api-design.md`](2026-03-28-auth-api-design.md).
