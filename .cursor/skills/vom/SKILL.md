...
**If the ticket is in state `review-pending`:** Use the **vom-plan-reviewer** subagent to perform the implementation review. Invoke it with the ticket ID (e.g. `/vom-plan-reviewer TKT-XXX` or "use the vom-plan-reviewer subagent to review TKT-XXX"). Do so **immediately in the same response**—do not wait for the user to ask. vom-plan-reviewer runs the full review workflow in its own context and reports back approved or changes-requested. Do not review and approve in the same flow—let the vom-plan-reviewer subagent do it.
...
**When you have just moved a ticket to `review-pending`** (e.g. after implementing): **invoke the vom-plan-reviewer subagent immediately in the same response**—do not stop and wait for the user to ask. Run vom-plan-reviewer so review happens without further instruction.
...
**Implementation review (review-pending):** Use the **vom-plan-reviewer** subagent for implementation review, including self-review. Launch it with the ticket ID (e.g. `/vom-plan-reviewer TKT-XXX`). vom-plan-reviewer runs in a separate context, verifies criteria and tests, then approves or requests changes and reports the outcome back. Prefer vom-plan-reviewer so review is consistent and isolated. **You may invoke vom-plan-reviewer in the same session**—it runs in a separate sub-session/context, so that is not "self-approve in the same flow" and is allowed and preferred.
